...
export function resolveAdaptiveText({ value, field, policy }) {
  ...
  for (let size = fontSize; size >= minFontSize; size -= 1) { const measured = measureWrappedText(value, { maxWidth, fontSize: size, lineHeight: size + lineSpacing }); if (measured.lines.length <= maxLines && measured.height <= maxHeight) return { text: measured.lines.join("\n"), fontSize: size, lineCount: measured.lines.length, height: measured.height, policy: "wrap-shrink" }; }
