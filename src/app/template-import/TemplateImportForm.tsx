...
export default function TemplateImportForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = await response.json().catch(() => ({})); setMessage(response.ok ? `Đã áp dụng ${payload.id}. Dùng template_id này cho lần render tiếp theo.` : payload.error || "Import thất bại.");
  }
