...
function applyUpsert(existing, sourceType, sourceId, batchId, item) {
  ...
  return {
    ...
    sourceItemId,
    batchId,
    templateKey: clean(item.template_key ?? existing?.templateKey ?? process.env.VOCABULARY_DEFAULT_TEMPLATE_ID ?? "vocabulary-editorial-v1", "template_key", 80),
    status,
  ...
  return serialized(async () => {
    ...
    const entry = existing
      ...
      : {
          ...
          templateKey: process.env.VOCABULARY_DEFAULT_TEMPLATE_ID ?? "vocabulary-editorial-v1",
          status: "published",
          word: "",
          meaning_vi: "",
          example_en: null,
